...
export const UserPreferences = sqliteTable("UserPreferences", {
  ...
  shoppingDay: text("shoppingDay").notNull().default("Saturday"),
  lowStockThreshold: text("lowStockThreshold").notNull().default("0.2"), // Store as string
  expiryWarningDays: integer("expiryWarningDays").notNull().default(3),

  // Boolean flags (stored as 0/1)
  expiryAlerts: integer("expiryAlerts", { mode: "boolean" })
    .notNull()
    .default(true),
  lowStockAlerts: integer("lowStockAlerts", { mode: "boolean" })
    .notNull()
    .default(true),
  shoppingReminders: integer("shoppingReminders", { mode: "boolean" })
    .notNull()
    .default(true),
  recipeSuggestions: integer("recipeSuggestions", { mode: "boolean" })
    .notNull()
    .default(true),
});
