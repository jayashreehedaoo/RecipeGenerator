...
/**
 * Extract recipe from URL (YouTube/Instagram) using AI
 * This function is called by AIRecipeModal in URL mode
 */
export async function extractRecipeFromUrlAction(
  url: string,
  contentText?: string
  ...
  try {
    ...
    try {
      ...
      if (!isYouTube && !isInstagram) {
        return {
          success: false,
          error: "Please provide a YouTube or Instagram recipe URL",
        };
      }
    } catch {
      return {
    ...
    await db.insert(schema.recipes).values({
      ...
      category: extractedRecipe.category || "Main Course",
      cuisine: extractedRecipe.cuisine || "",
      source: url, // Store the original URL
      isSaved: false,
      createdAt: now,
      updatedAt: now,
    });

    console.log("[Action] Recipe extracted and saved:", extractedRecipe.name);
